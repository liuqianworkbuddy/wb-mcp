...
/**
 * CallTool 参数到执行 argv 的最终归一。
 * 细粒度工具的 specArgv 已由 buildSpecArgv 安全组装，必须原样优先。
 */
export function callParamsToArgv({ specArgv, command, args } = {}) {
  if (Array.isArray(specArgv) && specArgv.length > 0) {
    return specArgv.map((value) => String(value));
  }
  return toArgv(command, args);
}
...
export async function buildServer({ version, readonly = false, logger } = {}) {
  ...
  const server = new Server(
    ...
    {
      instructions: 'AI 工作台 wb-cli 的 MCP 壳。录入路由：泛化记录指令（「帮我记录/用wb记一下」等未点名类型）一律走 wb_add（内置 AI 判类自动落表；调用方必须原样传递用户文本，不要总结、改写或润色；wb-cli 仅保留既有口语转写自动修正）；wb_note_add/wb_idea_add/wb_schedule_add 等细粒度工具仅当用户明确点名类型时使用。查询先用 wb_query（跨表检索）。图片上传一律走 wb_image_upload：path 传本机图片绝对路径，confirm 必须 true；成功后使用返回 JSON 的 data.url 作为公网 URL，可直发用户或写成 Markdown ![说明](url)，不要自行拼 URL。写操作必须传 confirm:true。',
      capabilities: { tools: {} },
    },
  );
  ...
  server.setRequestHandler(CallToolRequestSchema, async (req) => {
    ...
    const argv = callParamsToArgv({ specArgv, command, args });
    if (!argv) {
      return {
        content: [{
          type: 'text',
    ...
    try {
      ...
      if (r.code === 0) {
        ...
        // 人话反馈层：写命令成功 + 摘要器命中 → 得到大脑式摘要（文末仍附原始 JSON，
        // AI 消费方按 id/状态回写的链路不受影响）；未命中原样透传，读命令零改动。
        if (mayWrite) {
          const digest = humanize(argv.join(' '), shown);
          if (digest) return { content: [{ type: 'text', text: digest }] };
        }
