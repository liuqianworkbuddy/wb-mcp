...
export async function buildServer({ version, readonly = false, logger } = {}) {
  ...
  const server = new Server(
    ...
    {
      instructions: 'AI 工作台 wb-cli 的 MCP 壳。录入路由：泛化记录指令（「帮我记录/用wb记一下」等未点名类型）一律走 wb_add（内置 AI 判类自动落表；调用方必须原样传递用户文本，不要总结、改写或润色；wb-cli 仅保留既有口语转写自动修正）；wb_note_add/wb_idea_add/wb_schedule_add 等细粒度工具仅当用户明确点名类型时使用。查询先用 wb_query（跨表检索）。写操作必须传 confirm:true。',
      capabilities: { tools: {} },
    },
  );
