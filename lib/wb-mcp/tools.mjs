 ...
 *   wb_todo     todo / capsule / diary / idea / bug / book 待办闪念日记域
 *   wb_fin      fin 财务域
 *   wb_note     note 笔记域
...
export const DOMAIN_TIMEOUT = {
  ...
  dev_task: 60e3, vault: 60e3, archive: 60e3, image: 120e3,
};
...
export const DOMAIN_DESC = {
  ...
  todo: '待办与闪念日记域：todo/capsule/diary/idea/bug/book/schedule 命令族（列表/详情/完成/编辑/标签）。含写操作需 confirm。',
  ...
  image: '图床域：image upload 命令，把本机 PNG/JPG/GIF/WebP/BMP/AVIF 上传到 media-images/image-bed，返回公网 URL 与 Markdown 片段。写操作需 confirm。',
};
...
export const WRITE_TOOL_SPECS = [
  ...
  },
  {
    ...
    summary: '直录 AI 点子（锁定 idea 类型，但仍 AI 提炼标题/一句话；仅当用户明确要记点子时用，泛化记录走 wb_add）',
    params: [
      { name: 'raw', type: 'string', required: true, positional: true, maxLen: 2000, longInput: true, desc: '点子原文（含空格换行均可）' },
      { name: 'title', type: 'string', desc: '显式标题（缺省由 AI 从全文提炼；AI 不可用时本地降级截取）' },
    ],
  },
  {
    tool: 'wb_note_add', domain: 'note', argv: ['note', 'add'],
    summary: '写笔记（notes 表，#标签 自动解析；仅当用户明确要记笔记时用，泛化记录走 wb_add）',
    params: [
  ...
  },
  {
    ...
    summary: '新增日程（自然语言 AI 提炼标题/时间/地点，--at/--loc 显式覆盖；仅当用户明确要加日程时用，泛化记录走 wb_add）',
    params: [
  ...
  },
  {
    ...
    params: [
      ...
      { name: 'type', type: 'string', desc: '锁定类型别名（如 todo/bug/idea/schedule；仍会 AI 提炼标题与关键字段，仅跳过类型再判断）' },
    ],
  },
  {
  ...
  {
    tool: 'wb_image_upload', domain: 'image', argv: ['image', 'upload'],
    summary: '上传本地图片到图床并返回公网 URL。调用参数：path 传 Agent 本机图片文件绝对路径，confirm 必须 true。成功后读取结果 JSON 的 data.url 作为公网地址，可直接发给用户或写成 Markdown ![说明](url)；不要自行拼接或编造 URL。支持 PNG/JPG/GIF/WebP/BMP/AVIF，单文件 100MB，SVG 不支持',
    params: [{
      name: 'path',
      type: 'string',
      required: true,
      positional: true,
      desc: 'Agent 本机图片文件绝对路径，例如 /tmp/workbench-image.png；成功后使用返回 JSON 的 data.url',
    }],
  },
];
