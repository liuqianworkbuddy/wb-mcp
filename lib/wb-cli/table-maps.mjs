...
/**
 * 闪念 → capsules 行（纯文本；日记/笔记已拆独立主表）
 * tags：正文 hashtag 自动解析（A3 规范）——含 #标签 的胶囊保存后标签自动带上
 */
export function mapCapsule(c) {
  return {
    content: c._raw || '',
    title: c.title || '',
    category: 'inbox',
...
/** 日记 → diaries 行；entry_date 按 00:00-04:59 归前一天规则生成。 */
export function mapDiary(c) {
  const contentMd = c._raw || '';
  const createdAt = new Date().toISOString();
  return {
    content_md: contentMd,
    plain_text: markdownToPlainText(contentMd),
    title: c.title || '',
    entry_date: calcEntryDate(createdAt),
    category: '日记',
    tags: parseHashtags(contentMd),
    source: 'wb-cli',
    created_at: createdAt,
  };
}

/** 笔记 → notes 行；Markdown 为权威正文，plain_text 供检索。 */
export function mapNote(c) {
  const contentMd = c._raw || '';
  return {
    content_md: contentMd,
    plain_text: markdownToPlainText(contentMd),
