 ...
 * ------------------------------------------------------------
 * 域语义：notes 表（Markdown 权威正文 + plain_text 检索文本）
 *
 ...
 *
 * 列名陷阱（notes）：正文列是 content_md（非 content）；
 * 排序键 created_at；tags 为 text[] 且存储不带 # 前缀。
 */
...
import { markdownToPlainText } from '../records-core.mjs';

import { resolveLongInput } from './shared.mjs';
import { idempotencyLookup, idempotencySave } from './idempotency.mjs';
const LEGACY_NOTE_EXCEPTION_ID = '2d2031ef-cc5c-4ea9-a619-49494039b878';
...
function legacyExceptionToNote(row) {
  return {
    id: row.id,
    title: row.title || '',
    content_md: row.content || '',
    plain_text: row.content || '',
    category: row.category || '笔记',
    tags: Array.isArray(row.tags) ? row.tags : [],
    source: row.source || 'legacy-capsule',
...
async function fetchLegacyException(api) {
  try {
    const rows = await api('GET', `capsules?select=id,title,content,category,tags,source,created_at,updated_at&id=eq.${encodeURIComponent(LEGACY_NOTE_EXCEPTION_ID)}`);
    return rows?.length ? legacyExceptionToNote(rows[0]) : null;
  } catch {
    return null;
  }
}
...
/** note add "<内容>"：写入 notes 表，#tag 解析，回显确认。
...
async function noteAdd(ctx, flags, pos) {
  ...
  echo('将写入 · 笔记（notes · content_md + plain_text）');
  echo(`  → 标题：${title || '(空)'}`);
  echo(`  → 标签：${tags.length ? tags.map((t) => `#${t}`).join(' ') : '(无)'}`);
  echo(`  → 正文：${raw.length} 字`);
  if (!(await writeGuard(`新增笔记「${title || raw.slice(0, 20)}」`, flags))) return;

  const [created] = await ctx.api('POST', 'notes', row);
  await ctx.logCli('note_add', 'note', String(created.id),
    { title: row.title, content: raw, type: '笔记新增', table: 'notes' });

  out(flags, { note: created }, () => {
    console.log(`✅ 已写入 · 笔记（notes）`);
    console.log(`   [notes] ${created.title || '(无标题)'}  id=${created.id}`);
    console.log(`   /notes/ 前端列表已可见`);
  });
...
async function noteList(ctx, flags) {
  ...
  const list = await ctx.apiList('notes', {
    limit: flags.limit != null ? Number(flags.limit) : 20,
    cursor: typeof flags.cursor === 'string' && flags.cursor ? flags.cursor : null,
    all: !!flags.all,
    fields: typeof flags.fields === 'string' && flags.fields ? flags.fields : 'id,title,content_md,tags,category,created_at',
    order: 'created_at.desc,id',
    extra,
  });
  const notes = list.data;
  const legacy = await fetchLegacyException(ctx.api);
  const tagFilter = typeof flags.tag === 'string' && flags.tag.trim() ? normalizeTag(flags.tag) : null;
  if (
    legacy &&
    !notes.some((n) => n.id === legacy.id) &&
    (!tagFilter || (legacy.tags || []).includes(tagFilter))
  ) {
    notes.push(legacy);
    list.total = Number(list.total || 0) + 1;
  }
  out(flags, list, () => {
    if (!notes.length) {
...
async function noteShow(ctx, flags, pos) {
  ...
  const row = await findByIdPrefix(ctx.api, 'notes', id, 'id,title,content_md,category,tags,source,created_at,updated_at');
  out(flags, { note: row }, () => {
...
async function noteSearch(ctx, flags, pos) {
  ...
  let q = `notes?select=id,title,content_md,plain_text,tags,category,created_at&order=created_at.desc&limit=50`;
  ...
  const legacy = await fetchLegacyException(ctx.api);
  if (legacy && !rows.some((r) => r.id === legacy.id)) {
    const text = `${legacy.title}\n${legacy.plain_text}`;
    if (!kw || text.toLowerCase().includes(kw.toLowerCase())) rows = [legacy, ...rows];
  }
  if (typeof flags.tag === 'string' && flags.tag.trim()) {
  ...
  out(flags, { keyword: kw || null, tag: flags.tag || null, count: rows.length, notes: rows }, () => {
    ...
    for (const n of rows) {
      ...
      const content = n.content_md || n.plain_text || '';
...
/** note edit <id前缀> --file <md> | --title "新标题"：重解析替换 / 仅改标题 */
async function noteEdit(ctx, flags, pos) {
  ...
  if (!id || (!hasFile && !hasTitle)) {
    ...
    console.error('      --file：content_md/plain_text/title/tags 按文件重解析替换；--title：仅改标题');
    process.exitCode = 1;
    return;
  }
  const row = await findByIdPrefix(ctx.api, 'notes', id, 'id,title,content_md,category,tags');
  const patch = {};
  ...
  const changes = [];
  if (patch.content_md !== undefined) changes.push(`content_md→${patch.content_md.length} 字`);
  if (patch.plain_text !== undefined) changes.push('plain_text 已同步');
  ...
  const [updated] = await ctx.api('PATCH', `notes?id=eq.${encodeURIComponent(row.id)}&select=id,title,content_md,category,tags,created_at`, patch);
  await ctx.logCli('note_edit', 'note', row.id, {
    title: updated.title || row.title,
    content: patch.content_md !== undefined ? patch.content_md : `变更：${changes.join('，')}`,
    type: '笔记编辑', table: 'notes',
  });
...
async function noteTag(ctx, flags, pos) {
  ...
  const [updated] = await ctx.api('PATCH', `notes?id=eq.${encodeURIComponent(row.id)}&select=id,title,tags`, { tags: next });
  await ctx.logCli('note_tag', 'note', row.id, {
    title: row.title || (row.content_md || '').slice(0, 30),
    content: `标签：[${cur.join('、')}] → [${next.join('、')}]`,
    type: '笔记标签', table: 'notes',
  });
...
export const usage = [
  '  note add "<内容>"              记笔记（notes 表，#标签 自动解析，/notes/ 可见）[--yes]',
