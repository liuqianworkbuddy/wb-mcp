 ...
 */
export const FAMILY_WRITE = new Set(['add', 'fill', 'key', 'dev-task', 'idea', 'bug', 'book']);
...
const DOMAIN_FALLBACK = {
  ...
  fin: 'fin', note: 'note', vault: 'vault', archive: 'archive',
};
...
export function renderMd(cap) {
  ...
  for (const c of cap.commands) {
    ...
    lines.push(
      `| \`${c.name}\`${c.legacy ? '（legacy）' : ''} | ${c.domain ?? '—'} | ${c.write ? '✓' : '—'} | ${c.confirmNeed ? '✓' : '—'} | ${Array.isArray(c.resultFields) ? c.resultFields.join(' ') : '—'} | ${c.summary || ''} |`,
    );
