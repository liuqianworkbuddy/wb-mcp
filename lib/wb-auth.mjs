...
export function resolveAuth(opts = {}) {
  ...
  if (process.env.WB_API_KEY && process.env.WB_API_KEY.startsWith('wbk_')) {
    return { mode: 'gateway', key: process.env.WB_API_KEY, profile, source: 'env:WB_API_KEY' };
  }
  ...
  if (existsSync(agentEnv)) {
    const k = envMatch(readFileSync(agentEnv, 'utf8'), ['WB_API_KEY']);
    if (k && k.startsWith('wbk_')) {
      return { mode: 'gateway', key: k, profile, source: `file:${agentEnv}` };
    }
  }
  ...
  if (profile !== 'workbuddy') {
    ...
    if (existsSync(mainEnv)) {
      const k = envMatch(readFileSync(mainEnv, 'utf8'), ['WB_API_KEY']);
      if (k && k.startsWith('wbk_')) {
        return { mode: 'gateway', key: k, profile, source: `file:${mainEnv}` };
      }
    }
  }
  ...
  if (process.env.CLOUDBASE_API_KEY) {
    return { mode: 'legacy', key: process.env.CLOUDBASE_API_KEY, profile, source: 'env:CLOUDBASE_API_KEY' };
  }
  const envPath = resolve(homedir(), '.workbuddy/cloudbase.env');
  if (existsSync(envPath)) {
    const txt = readFileSync(envPath, 'utf8');
    const api = envMatch(txt, ['CLOUDBASE_API_KEY']);
    if (api) return { mode: 'legacy', key: api, profile, source: `file:${envPath}` };
    const pub = envMatch(txt, ['(?:CLOUDBASE_)?PUBLISHABLE_KEY']);
    if (pub) return { mode: 'legacy', key: pub, profile, source: `file:${envPath}(publishable)` };
  }
  const cfgPath = join(REPO_ROOT, 'lib', 'cloudbase-config.ts');
  if (existsSync(cfgPath)) {
    const m = readFileSync(cfgPath, 'utf8').match(/PUBLISHABLE_KEY\s*=\s*\n?\s*'([^']+)'/);
    if (m) return { mode: 'legacy', key: m[1], profile, source: 'lib/cloudbase-config.ts' };
  }
  return { mode: 'legacy', key: FALLBACK_KEY, profile, source: 'builtin-fallback(anon,RLS 后不可用)' };
}
